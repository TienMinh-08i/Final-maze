...
class MDP:
    ...
    def get_action_at_position(self, state):
        return self.actions
...
def policy_iteration(mdp, gamma=0.99, theta=0.0001):
    """
    ...
    def improve_policy(V, gamma):
        ...
        for s in mdp.states:
            ...
            for a in mdp.get_action_at_position(s):
                action_value = 0
                for s_prime in mdp.states:
                    transition_prob = mdp.get_transition_prob(s, a, s_prime)
                    reward = mdp.get_reward(s, a, s_prime)
    ...
    policy = {s: random.choice(mdp.get_action_at_position(s)) for s in mdp.states}
...
myMaze = Maze().load(r"C:\Users\admin\Desktop\src\maze\maze_16x16.pkl")
env = MazeEnv(myMaze)
...
# Lưu policy vào file
with open(r"C:\Users\admin\Desktop\src\pi4x4", 'wb') as f:
    pickle.dump(optimal_policy, f)

# Lấy policy từ file
with open(r"C:\Users\admin\Desktop\src\pi4x4", 'rb') as f:
